...
contract StabiliteBondingCalculator is IBondingCalculator {
  ...
  function markdown(address _pair) external view returns (uint256) {
    ...
    uint256 reserve;
    if (IUniswapV2Pair(_pair).token0() == STABIL) {
      reserve = reserve1;
    } else {
      reserve = reserve0;
    }
    return
      reserve.mul(2 * (10**IERC20(STABIL).decimals())).div(
        getTotalValue(_pair)
      );
  }
}
