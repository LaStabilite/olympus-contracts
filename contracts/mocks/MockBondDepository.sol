...
contract MockStabiliteBondDepository is Ownable {
  ...
  address public immutable treasury; // mints STABIL when receives principle
  ...
  function deposit(
    ...
    /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) STABIL
         */
    ...
    if (fee != 0) {
      ...
      IERC20(STABIL).safeTransfer(DAO, fee);
    }
  ...
  function stakeOrSend(
    ...
    if (!_stake) {
      ...
      IERC20(STABIL).transfer(_recipient, _amount); // send payout
    } else {
  ...
  /**
   *  @notice allow anyone to send lost tokens (excluding principle or STABIL) to the DAO
   *  @return bool
   */
