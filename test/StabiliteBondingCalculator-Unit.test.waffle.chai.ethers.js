...
describe('StabiliteBondingCalculator', () => {
  ...
  beforeEach(
    async function () {
      ...
      tt1 = await DAIContract.connect(deployer).deploy(1);

      FRAXContract = await ethers.getContractFactory('FRAX');
      tt2 = await FRAXContract.connect(deployer).deploy(1);
      ...
      BondingCalcContract = await ethers.getContractFactory('StabiliteBondingCalculator');
