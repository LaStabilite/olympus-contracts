...
describe("Stabilite Staking", () => {
  ...
  beforeEach(async () => {
    ...
    OLY = await ethers.getContractFactory("StabiliteERC20Token");
    oly = await OLY.deploy();

    Staking = await ethers.getContractFactory("StabiliteStaking");
    ...
    sOLY = await ethers.getContractFactory("sStabilite");
    ...
    const signatureAddr1 = await signerAddr1._signTypedData(
      domainAddr1,
      types,
      valAddr1
    );
  ...
  describe("stakeOLY()", () => {
    ...
    it("should not let a user stake if they are not the proper signer", async () => {
      ...
      await expect(
        staking
          .connect(addr1)
          .stakeOLY("500000000", deadline, _sig.v, _sig.r, _sig.s)
      ).to.be.revertedWith("revert ZeroSwapPermit: Invalid signature");
    ...
    it("should not let a user stake more than they have", async () => {
      ...
      await expect(
        staking.stakeOLY("10000000000000001", deadline, sig.v, sig.r, sig.s)
    ...
    it("should distirbute profits correctly if someone stakes after epoch", async () => {
      ...
      await staking
        .connect(addr1)
        .stakeOLY(3000000000, deadline, sigAddr1.v, sigAddr1.r, sigAddr1.s);
      expect(await soly.balanceOf(deployer.address)).to.equal(tokenAmount);
    ...
    it("should rebase a single user correctly when profits are distributed", async () => {
      ...
      await treasury.sendOLYProfits(2000000000);
      await treasury.sendOLYProfits(1000000000);
      expect(await soly.balanceOf(deployer.address)).to.equal(
        +2000000000 + +tokenAmount
      );
    });
    ...
    it("should add circulating supply correctly when a rebase is made to distribute profits", async () => {
      ...
      await treasury.sendOLYProfits(2000000000);
      await treasury.sendOLYProfits(1000000000);
      expect(await soly.circulatingSupply()).to.equal(
        +2000000000 + +tokenAmount
      );
    });
    ...
    it("should pass if there is profit and no staker", async () => {
      ...
      expect(totalSupplyAfter2).to.equal(
        totalSupplyBefore.toNumber() + 3000000000
      );
    });
  ...
  describe("unstakeOLY()", () => {
    ...
    it("should transfer OLY back to user when they unstake", async () => {
      ...
      await staking.unstakeOLY("500000000", deadline, _sig.v, _sig.r, _sig.s);
      expect(await oly.balanceOf(deployer.address)).to.equal(
        +balanceAfter.toNumber() + +500000000
      );
      ...
      await staking.unstakeOLY(500000000, deadline, _sig2.v, _sig2.r, _sig2.s);
      expect(await oly.balanceOf(deployer.address)).to.equal(
        balanceAfter.toNumber() + 1000000000
      );
    });
    ...
    it("shoud NOT let a user unstake more than they have", async () => {
      ...
      await expect(
        staking.unstakeOLY(1000000001, deadline, _sig.v, _sig.r, _sig.s)
    ...
    it("should not let a user unstake if they are not the approved signer", async () => {
      ...
      await expect(
        staking
          .connect(addr1)
          .stakeOLY(tokenAmount, deadline, _sig.v, _sig.r, _sig.s)
      ).to.be.revertedWith("revert ZeroSwapPermit: Invalid signature");
  ...
  describe("distributeOLYProfits()", () => {
    it("should NOT let non owner distribute", async () => {
      await expect(staking.distributeOLYProfits()).to.be.revertedWith(
